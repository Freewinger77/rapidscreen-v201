    ...
    for (const job of jobs) {
      try {
        ...
        if (candError) {
          ...
          jobsWithCandidates.push(convertJobFromDB(job, []));
          continue;
        }

        // Load notes for each candidate
        const candidatesWithNotes = [];
        ...
        for (const candidate of (candidates || [])) {
          try {
            ...
            console.error('Error loading notes for candidate:', err);
            candidatesWithNotes.push({
              id: candidate.id,
              name: candidate.name,
              phone: candidate.phone,
        ...
        jobsWithCandidates.push(convertJobFromDB(job, candidatesWithNotes));
      } catch (err) {
...
export async function loadCampaigns(): Promise<Campaign[]> {
  try {
    ...
    for (const campaign of campaigns) {
      try {
        const { data: targets } = await supabase.from('campaign_targets').select('*').eq('campaign_id', campaign.id);
        const { data: matrices } = await supabase.from('campaign_matrices').select('*').eq('campaign_id', campaign.id);
        const { data: campaignCandidates } = await supabase.from('campaign_candidates').select('*').eq('campaign_id', campaign.id);

        campaignsWithData.push({
          id: campaign.id,
          backendCampaignId: campaign.backend_campaign_id || null,
          name: campaign.name || 'Unnamed Campaign',
          jobId: campaign.job_id,
          jobTitle: campaign.job_title || '',
          linkJob: campaign.link_job || null,
          ...
          candidates: (campaignCandidates || []).map((c: any) => ({
            id: c.id,
            forename: c.forename || 'Unknown',
            surname: c.surname || '',
            telMobile: c.tel_mobile || '',
            email: c.email || null,
            callStatus: c.call_status || 'not_called',
            availableToWork: c.available_to_work || null,
            interested: c.interested || null,
            knowReferee: c.know_referee || null,
        ...
        // Skip this campaign if it fails
      }
    }
...
export async function addCampaign(campaign: Omit<Campaign, 'id'>): Promise<string> {
  const { data, error} = await supabase
    ...
    .insert({
      name: campaign.name,
      backend_campaign_id: (campaign as any).campaignId || null,  // Store backend ID!
  ...
  if (campaign.candidates && campaign.candidates.length > 0) {
    await supabase.from('campaign_candidates').insert(
      campaign.candidates.map((c) => ({
        ...
        email: c.email || null,
        call_status: c.callStatus || 'not_called',
        available_to_work: c.availableToWork,
        interested: c.interested,
        know_referee: c.knowReferee,
...
export async function updateCampaign(
  ...
  console.log('📝 updateCampaign called with:', { campaignId, updates });
